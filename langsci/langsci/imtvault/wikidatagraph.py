...
def process(ID, first=False):
    ...
    if (
        first
    ):  # we only allow instance_of at leaves of the tree, otherwise we get "class is_instance of metaclass" predicates, which we do not want.
    ...
    new279s = [
        p for p in p279s if p not in d and p not in initqueue and p not in nonsense
    ]
    ...
    new31s = [
        p for p in p31s if p not in d and p not in initqueue and p not in nonsense
    ]
