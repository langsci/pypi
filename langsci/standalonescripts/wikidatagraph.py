...
def process(ID, first=False):
    ...
    new279s = [p for p in p279s if p not in d and p not in initqueue and p not in nonsense]
    queuelength += len(new279s)
    if p31s is None:
        p31s = []
    # setup recursion for categories; skip items we already know or which are already in the queue
    new31s = [p for p in p31s if p not in d and p not in initqueue and p not in nonsense]
