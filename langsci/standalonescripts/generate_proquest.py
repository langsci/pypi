...
today = "2022-08-08" #FIXME
publication_date = "1970-01-01" #FIXME
isbn_digital =  "978"
issn =  "1234-4321"
metalanguage = "eng"
...
try:
    role = authorrolecode
    authorstring = "\n".join([creatortemplate % (i+1,role,creator[0],creator[1],creator[2]) for i, creator in enumerate(metadata["creators"]["authors"]) ])
except TypeError:
    pass
try:
    role = editorrolecode
    editorstring = [creatortemplate % (i+1,role,creator[0],creator[1],creator[2]) for i, creator in enumerate(metadata["creators"]["editors"]) ]
except TypeError:
    pass

creatorstring = authorstring + editorstring

assert creatorstring != ''
